      ...
      body: Column(
        ...
        children: [
          ...
          const Padding(
            ...
            child: Text("Find Your\nDream Chess", style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: kDarkBlue, height: 1.2)),
          ),
              ...
              children: [
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    prefixIcon: Padding(padding: const EdgeInsets.only(left: 20.0, right: 12.0), child: Icon(Icons.search, color: kDarkBlue.withOpacity(0.8), size: 24)),
                ...
                Padding(
                  ...
                  child: InkWell(
                    ...
                    child: Container(
                      ...
                      child: Icon(Icons.tune, color: kDarkBlue.withOpacity(0.9), size: 22.0),
                    ),
                  ),
                ),
              ],
...
class _DiscountBannerSectionState extends State<DiscountBannerSection> {
  ...
  final List<Map<String, String>> _bannerItems = [
    {"title": "50% OFF", "subtitle": "05 - 10 July", "imageUrl": "https://m.media-amazon.com/images/I/81M7o+-V3CL._AC_SL1500_.jpg"},
    {"title": "NEW ARRIVAL", "subtitle": "Glass Chess Sets", "imageUrl": "https://m.media-amazon.com/images/I/71v5Xyol6qL._AC_SL1500_.jpg"},
  ];
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        children: [
          SizedBox(height: 120, child: PageView.builder(controller: _pageController, itemCount: _bannerItems.length, onPageChanged: (page) => setState(() => _currentPage = page), itemBuilder: (context, index) => Padding(padding: const EdgeInsets.symmetric(horizontal: 16.0), child: _buildBannerCard(banner: _bannerItems[index])))),
          const SizedBox(height: 12),
          Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(_bannerItems.length, (index) => _buildDot(index: index))),
        ],
      ),
    );
  }
  ...
  Widget _buildBannerCard({required Map<String, String> banner}) {
    return Container(
      ...
      decoration: BoxDecoration(color: kPrimaryBlue, borderRadius: BorderRadius.circular(20.0), boxShadow: [BoxShadow(color: kPrimaryBlue.withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 4))]),
      child: Row(
        children: [
          Expanded(flex: 5, child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [Text(banner['title']!, style: const TextStyle(fontSize: 26, fontWeight: FontWeight.bold, color: Colors.white)), const SizedBox(height: 4), Text(banner['subtitle']!, style: TextStyle(fontSize: 16, color: Colors.white.withOpacity(0.8)))]),),
          const SizedBox(width: 16),
          Expanded(flex: 4, child: ClipRRect(borderRadius: BorderRadius.circular(12.0), child: Image.network(banner['imageUrl']!, fit: BoxFit.cover, height: double.infinity, errorBuilder: (c, e, s) => Container(color: kLightBlue.withOpacity(0.5))))),
        ],
      ),
    );
  }
  ...
  Widget _buildDot({required int index}) {
    ...
    return AnimatedContainer(duration: const Duration(milliseconds: 200), margin: const EdgeInsets.symmetric(horizontal: 4.0), height: 8.0, width: isActive ? 24.0 : 8.0, decoration: BoxDecoration(color: isActive ? kPrimaryBlue : kLightBlue, borderRadius: BorderRadius.circular(4)));
  }
}
...
class CategoryAndProductSection extends StatefulWidget {
  ...
  @override
  State<CategoryAndProductSection> createState() => _CategoryAndProductSectionState();
}
  ...
  void _applyFilter() {
    setState(() {
      if (_selectedFilter == "All Product") {
        ...
        _filteredProducts = _allProducts.where((p) => p['category'] == _selectedFilter).toList();
      }
    });
  }
                ...
                child: Column(
                  ...
                  children: [
                    Column(
                      ...
                      children: [
                        Text(widget.product['name']!, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: kVeryLightBlue), maxLines: 2, overflow: TextOverflow.ellipsis),
                        const SizedBox(height: 2),
                        Text(widget.product['subtitle'] ?? 'Item', style: TextStyle(fontSize: 11, color: kVeryLightBlue.withOpacity(0.8)), maxLines: 1, overflow: TextOverflow.ellipsis),
                      ],
                    ),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      ...
                      children: [
                        Text("Rp. ${formatCurrency.format(widget.product['price'])}", style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Colors.white)),
                        InkWell(
                          ...
                          child: Container(
                            ...
                            child: Icon(_isLiked ? Icons.favorite : Icons.favorite_border, color: _isLiked ? Colors.redAccent : kVeryLightBlue, size: 18),
                          ),
                        ),
                      ],
                    )
